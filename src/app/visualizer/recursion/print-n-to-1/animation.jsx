...
import { generatePrintNTo1Frames } from "@/features/algorithms/recursion/printNTo1Logic";
...
const PrintNTo1Animation = () => {
  ...
  const frames = useMemo(() => {
    ...
    return Array.from(generatePrintNTo1Frames(n));
  }, [nVal, isVisualizing]);
    ...
    <main className="container mx-auto">
      ...
      <div className="max-w-4xl mx-auto grid gap-6 md:grid-cols-12 mb-8">
        ...
        <div className="md:col-span-4 rounded-xl bg-white dark:bg-[#1a1a1a] p-5 shadow border border-gray-200 dark:border-zinc-800 flex flex-col items-center">
          ...
          <div className="w-full flex-1 flex flex-col items-center justify-center gap-2 p-4 bg-zinc-950 text-[#a435f0] font-mono text-lg rounded-xl min-h-64 shadow-inner border border-zinc-800">
            {printedList.length === 0 ? (
              <span className="text-zinc-600 text-xs">No output printed yet</span>
            ) : (
              <div className="flex flex-col gap-1 items-center">
                {printedList.map((val, idx) => (
                  <span key={idx} className="animate-fade-in font-bold">
                    &gt; {val}
                  </span>
                ))}
              </div>
